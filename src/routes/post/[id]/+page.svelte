...
<svelte:head>
	...
	<meta
		name="description"
		content={postData.caption || `Post by ${postData.user.display_name} on Likey`}
	/>
</svelte:head>
...
<div class="min-h-screen bg-base-100">
	...
	<div class="sticky top-0 z-10 border-b border-base-300 bg-base-100 p-4">
		<div class="flex items-center gap-4">
			<button onclick={() => history.back()} class="btn btn-circle btn-ghost">
				<ArrowLeft size={20} />
			</button>
	...
	<div class="container mx-auto max-w-2xl px-4 py-6">
		<article class="card border border-base-300 bg-base-100 shadow-lg">
			...
			<div class="flex items-center gap-4 border-t border-base-300 px-4 py-3">
				...
				<!-- Comment Button -->
				<button class="btn gap-2 btn-ghost btn-sm" disabled>
					<MessageCircle size={20} />
					{postData.comment_count}
				</button>

				<!-- Share Button -->
				<button class="btn gap-2 btn-ghost btn-sm" onclick={sharePost} title="Share this post">
					<Share2 size={20} />
				</button>
			</div>
		</article>
		...
		<div class="card mt-6 border border-base-300 bg-base-100 shadow-lg">
			<div class="p-4">
				...
				{:else}
								...
								<div class="flex gap-3">
									<a href="/profile/{comment.user.username}" class="avatar">
										<div class="h-8 w-8 rounded-full">
											...
											{:else}
												<div
													class="flex h-full w-full items-center justify-center rounded-full bg-primary text-sm text-primary-content"
												>
					...
					<!-- Add Comment Form -->
					{#if $user}
						<form onsubmit={addComment} class="mt-6 flex gap-3 border-t border-base-300 pt-4">
							<div class="avatar">
								<div class="h-8 w-8 rounded-full">
					...
					{:else}
						<div class="py-6 text-center text-base-content/60">
							<p>
								Please <button class="link" onclick={() => showLogin.set(true)}>sign in</button> to comment
							</p>
						</div>
					{/if}
				{/if}
