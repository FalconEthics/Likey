<script>
	...
	function getFollowButtonProps(user) {
		...
		if (isFollowing) {
			return { text: 'Following', icon: UserCheck, classes: 'btn-outline hover:btn-error hover:text-error' };
		}
...
<svelte:head>
	<title>{data.profile.display_name}'s {data.type === 'followers' ? 'Followers' : 'Following'} - Likey</title>
</svelte:head>
					...
					<div class="flex items-center gap-3 p-3 rounded-lg hover:bg-base-200 transition-colors">
						...
						<div class="flex-1 min-w-0">
							<a href="/profile/{user.username}" class="block">
								<h3 class="font-semibold text-sm truncate hover:underline">
									{user.display_name}
								</h3>
								<p class="text-xs text-base-content/60 truncate">@{user.username}</p>
								{#if user.bio}
									<p class="text-xs text-base-content/80 mt-1 line-clamp-2">{user.bio}</p>
								{/if}
							</a>
						</div>
